...
int score, level, rockRate, lpower;
boolean play;
Timer lTime, puTime; //l = r
...
void setup() {
  ...
  lTime.start();
  puTime = new Timer(int(random(1000, 2000)));
  puTime.start();
  laser = new SoundFile(this, "laserSound.wav");
  fart = new SoundFile(this, "fartt.mp3");
}
...
void infoPanel() {
  ...
  text("Health: " + s1.health, 240, 36);
  text("Level: " + level, 420, 36);
  text("Ammo: " + s1.laserCount, 600, 36);
}
...
void startScreen() {
  ...
  text("The power ups are completely random, but they all guarentee a health boost, ammo boost, and one special other effect", width/2, 300);
  text("You'll never know (teehee)", width/2, 320);
  text("One powerup will give you another turret, one will give a massive buff to many things, one increases damage, and one is useless", width/2, 340);
